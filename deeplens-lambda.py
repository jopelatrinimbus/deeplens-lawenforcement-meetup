...
class LocalDisplay(Thread):
    ...
    def frame2jpeg(self, frame):
        ret, jpeg = cv2.imencode('.jpg', cv2.resize(frame, self.resolution))
        if not ret:
            raise Exception('Failed to convert frame to jpeg image')
    ...
    try:
        ...
        while True:
            ...
            if face_present:
                ...
                s3.put_object(Bucket='deepenforcement-candidates', Body=candidate, Key=key_name)
                client.publish(topic=iot_topic, payload='uploading candidate file to s3')
